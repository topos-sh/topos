...
- `custody/upload.rs` — the candidate DTOs. `custody/lifecycle.rs` + its db twin — the
  object-lifecycle fence: quarantine ingest (server rehash — no client id trusted),
  lease-before-migrate, durable install, the fenced `object_presence` state machine, tombstones.
- `custody/commit.rs` / `db/custody/pointer.rs` — version rows + the generation-fenced pointer
  ...
  per-workspace storage stat. `custody/gc.rs` — mark-then-acquire GC, recovery, the quarantine
  janitor (`run_gc`/`run_recovery`/`run_janitor` — the composing server MUST schedule them; no
  scheduler here). Clock convention: epoch milliseconds.
