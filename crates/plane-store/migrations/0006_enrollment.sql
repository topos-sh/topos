...
-- edits an applied migration's bytes — sqlx checksums the file). Same posture as 0001-0005: STRICT + WITHOUT
-- ROWID, every row `workspace_id`-scoped, content ids the raw 32-byte sha256 BLOBs the kernel passes,
-- width-checked. Every opaque credential is stored ONLY as its sha256 (the plaintext is HMAC-derived from a
-- 0600 enrollment secret and never persisted), so a database read can never recover a live credential and a
-- revoke is an instant server-side row flip. Time columns are epoch MILLISECONDS (the one server-clock unit,
-- `wire::now_utc()`), matching the read-token expiry this migration adds.
...
-- row, so the 200+ publish/read tests stay green). `deployment_mode` decides the redeem gate (cloud requires
...
-- the enrollment flow. `revoked` is the instant server-side kill switch; `expires_at` (epoch-ms, nullable =
-- never expires) is the soft deadline. `created_by` is the inviting owner principal (audit).
...
CREATE TABLE enrollment_grants (
    ...
    workspace_id   TEXT    NOT NULL,
    invite_sha256  BLOB             CHECK (invite_sha256 IS NULL OR length(invite_sha256) = 32),
    principal      TEXT    NOT NULL,
...
CREATE TABLE device_auth_sessions (
    device_code_sha256  BLOB    NOT NULL CHECK (length(device_code_sha256) = 32),
...
CREATE TABLE passcodes (
    device_code_sha256 BLOB    NOT NULL CHECK (length(device_code_sha256) = 32),
    principal          TEXT    NOT NULL,
    passcode_sha256    BLOB    NOT NULL CHECK (length(passcode_sha256) = 32),
    expires_at         INTEGER NOT NULL,
...
ALTER TABLE read_token ADD COLUMN expires_at INTEGER;
