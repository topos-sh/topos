...
-- migration's bytes — sqlx checksums the file). Same posture as 0001-0003: every row `workspace_id`-scoped,
-- content ids the raw 32-byte sha256 BYTEA the kernel passes, width-checked, and the `(epoch, seq)` columns
-- carry the JCS / I-JSON safe-integer ceiling (2^53 - 1) the pointer preimage enforces.

-- PROPOSALS — a `publish --propose` candidate. `status` is the stored fact ({open, accepted, rejected}),
-- transitioned only under the one serializable write transaction (the Postgres realization also takes a
-- `SELECT … FOR UPDATE` on this row). **`stale` is NEVER stored — it is the derived fact `status='open' AND
...
-- already the idempotency key), so the server mints nothing. It is NOT keyed on `(commit_id, base_es)`: a
-- rejected resubmit of identical bytes is the SAME content-derived `commit_id`, so that would resurrect/
-- collide a terminal row — the surrogate `id` keeps each attempt distinct while the partial index below
-- enforces one LIVE proposal per candidate+base.
--
