...
-- every query binds it (isolation is this binding, never a directory). Content ids are the raw 32-byte
-- sha256 BYTEA the kernel/git layer pass, width-checked. This layer keeps everything in the git store
-- (`location` is always `git`); the size-routed large-object store adds the `large-local` route and the
-- `location`/`size`/`git_oid` columns are shaped for it.
...
-- `location` records WHICH physical store holds the bytes (`git` or the large-object store). `git_oid` is
-- the physical LOCATOR for a `git` object — the 20-byte git object id of the loose blob — set in the same
-- transaction that flips the row to `present`, so a GC can always find the bytes to unlink (the database,
-- not a ref, owns the locator). `size` is OPERATIONAL only (accounting + the size-routing); it NEVER enters
-- the canonical manifest, the digest, or any id. `status_updated_at` stamps every transition so the recovery
-- sweep can finalize only a STALE `deleting` (one a crashed GC left behind) without racing a live GC.
...
-- NULL so the migrated version stays rooted until the later pointer-move consumes it.
