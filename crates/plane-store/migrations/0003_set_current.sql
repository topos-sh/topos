...
-- migration's bytes — sqlx checksums the file). Same posture as 0001/0002: every row `workspace_id`-scoped,
-- content ids the raw 32-byte sha256 BYTEA the kernel passes, width-checked. The `(epoch, seq)` columns carry
-- the JCS / I-JSON safe-integer ceiling (2^53 − 1) the pointer preimage enforces, so a generation a follower
-- could never verify can never be stored.
...
-- flip between the two). The `review_required` boolean is modelled as a BIGINT 0/1 (the Rust reads `i64 != 0`).
CREATE TABLE workspace_policy (
    workspace_id    TEXT   NOT NULL,
    review_required BIGINT NOT NULL DEFAULT 0 CHECK (review_required IN (0, 1)),
...
-- verifies the device-op signature and checks the principal is rostered — all inside the one serializable
-- write transaction, so a revoke committed BEFORE the promotion is serialized AHEAD of it and blocks the
-- move. `public_key` is the raw 32-byte Ed25519 verifying key; `revoked` is a BIGINT 0/1.
...
-- REVERT support. `revert --to <good>` builds a forward commit `{tree: good.tree, parents: [current]}`, which
-- needs good's `bundle_digest` to re-derive the new version_id — but the git commit object does NOT persist
-- it (it is only an INPUT to the version_id/render check, never stored). So the pointer-move records the
-- server-rehashed digest here, on the commit's provenance row, at promote/record time; revert reads it O(1).
-- Nullable: legacy rows written before the pointer-move carry NULL and cannot be revert targets (a typed
-- failure), which is correct — every version born through the pointer-move path carries its digest. An
-- octet_length CHECK pins the width for the non-NULL case.
ALTER TABLE skill_commit ADD COLUMN bundle_digest BYTEA CHECK (bundle_digest IS NULL OR octet_length(bundle_digest) = 32);
