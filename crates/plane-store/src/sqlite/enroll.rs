...
impl Db {
    ...
    pub(crate) async fn poll_txn(
        ...
        let r = poll_run(&mut tx, device_code_sha256, now, created_at, secret).await;
        match r {
            Ok(poll) => {
...
impl Db {
    ...
    pub(crate) async fn complete_passcode_txn(
        ...
        let mut tx = self.begin_immediate().await?;
        let r = complete_passcode_run(&mut tx, user_code, principal, code, now).await;
        match r {
            Ok(out) => {
                tx.commit().await.map_err(AuthorityError::internal)?;
                Ok(out)
...
impl Db {
    ...
    pub(crate) async fn redeem_txn(
        ...
        let mut tx = self.begin_immediate().await?;
        let r = redeem_run(&mut tx, input, secret).await;
        match r {
            Ok(out) => {
                tx.commit().await.map_err(AuthorityError::internal)?;
                Ok(out)
...
async fn read_grant(tx: &mut Transaction<'_, Sqlite>, gs: &[u8]) -> Result<Option<GrantRow>> {
    let row = sqlx::query!(
...
async fn read_grant_skills(tx: &mut Transaction<'_, Sqlite>, gs: &[u8]) -> Result<Vec<SkillId>> {
    let rows = sqlx::query!(
...
impl Db {
    ...
    pub(crate) async fn create_invite_txn(
        ...
        let mut tx = self.begin_immediate().await?;
        let r = create_invite_run(&mut tx, input, invite_token_sha256).await;
        match r {
            Ok(out) => {
                tx.commit().await.map_err(AuthorityError::internal)?;
                Ok(out)
    ...
    pub(crate) async fn governance_mutation_txn(
        ...
        let mut tx = self.begin_immediate().await?;
        let r = governance_mutation_run(&mut tx, input).await;
        match r {
            Ok(out) => {
                tx.commit().await.map_err(AuthorityError::internal)?;
                Ok(out)
