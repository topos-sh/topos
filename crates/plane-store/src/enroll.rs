//! The enrollment + governance issuance core — the orchestration half (outside the transaction).
//!
//! This is where a real workspace stands up, where a device proves possession of the key it registers, and
//! where the plane mints the only credentials it ever issues: **workspace-scoped** invites, enrollment
//! grants, and per-skill read tokens — **never** a user OAuth token. Every issuance / roster / role / device
//! decision is made INSIDE these ops against a server-trusted row; a client-asserted id is never parsed into
//! authority. Every opaque credential is **deterministically HMAC-derived** from a `0600` enrollment secret
