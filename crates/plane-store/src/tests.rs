...
async fn resolve_read_token_resolves_a_scope_and_a_miss_is_notfound() {
    ...
    let scope = a.resolve_read_token("tok-secret-123").await.unwrap();
    ...
    assert!(matches!(
        a.resolve_read_token("tok-WRONG").await,
        Err(AuthorityError::NotFound)
    ));
}
...
async fn read_current_present_absent_and_corrupt_blob() {
    ...
    fx.authority
        ...
        .await
        .unwrap();
    let scope = fx.authority.resolve_read_token("tok-cur").await.unwrap();
...
async fn serve_object_serves_in_scope_and_rejects_a_scope_or_path_mismatch() {
    ...
    a.db()
        ...
        .await
        .unwrap();
    let scope = a.resolve_read_token("tok-serve").await.unwrap();
...
async fn read_version_metadata_accepted_proposal_arm_and_unauthorized() {
    ...
    fx.authority
        ...
        .await
        .unwrap();
    let scope = fx.authority.resolve_read_token("tok-vm").await.unwrap();
    ...
    fx.authority
        ...
        .await
        .unwrap();
    let outscope = fx.authority.resolve_read_token("tok-out").await.unwrap();
    assert!(matches!(
        fx.authority
