...
import { connectSession, sessionUserByCredential } from "@/lib/db/identity.server";
 ...
 * ACTIVE session on THIS server — the acting session's own workspace is irrelevant; what
 * matters is the resolved PERSON, and SEAT STANDING in the target workspace is the trust
 * basis: the transaction locks that seat and mints only where one stands. Anything else —
 * bad bearer, unknown or unseated workspace — is the ONE uniform 404 (never an existence
 * oracle). The plaintext credential is returned exactly once over this authenticated
 * exchange, the same practice as the flow start returning its code.
 */
...
export async function action({ request }: ActionFunctionArgs): Promise<Response> {
  ...
  const actor = await sessionUserByCredential(credential);
  if (actor === null) {
    return uniformNotFound();
  }
  const minted = await connectSession(actor, body.workspace, body.requested_name.trim());
  if (minted === null) {
    return uniformNotFound();
  }
