 ...
 *
 *  - `publicOrigin` — the origin a shareable resource address rides (`topos login
 *    <origin>/<address>`, the invite mail, the settings page's address block). The app serves those
 ...
 *    `/api` mount, where the session lane (`/api/v1/…`) is served. The CLI joins `/v1/…` paths onto
 ...
 * e2e) leave it unset and the request origin is correct.
 */
...
export function apiBase(request: Request): string {
  return `${publicOrigin(request)}/api`;
}
