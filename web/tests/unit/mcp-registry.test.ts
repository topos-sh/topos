...
async function seedServer(
  ...
  await db.q(
    `INSERT INTO web.mcp_server (id, workspace_id, name, display_name, auth_mode, auth_note, status)
...
async function seedRevision(
  ...
  await db.q(
    `INSERT INTO web.mcp_server_revision
     ...
     VALUES ($1, $2, $3, $4, $5::jsonb, 'streamable-http', 'https://x.example/mcp', now(), 'Staff')`,
    [id, serverId, seq, String(doc.version), JSON.stringify(doc)],
  );
