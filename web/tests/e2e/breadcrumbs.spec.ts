...
test("the your-devices page shows workspace → Your devices (the account tail off a workspace root)", async ({
  page,
}) => {
  const ws = await theWorkspace();
  await gotoSettled(page, "/account/devices");

  const bc = breadcrumb(page);
  await expect(bc).toBeVisible();
  // your-devices is top-level (an account page), yet the single-tenant chrome still scopes the sole
  // workspace, so the root crumb precedes the account tail.
  await expect(bc.getByRole("link", { name: ws.displayName })).toBeVisible();
  await expect(bc.locator('[aria-current="page"]')).toHaveText("Your devices");
});
