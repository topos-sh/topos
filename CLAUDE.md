...
| Client CLI | Built on the MANIFEST architecture: a scope IS a `topos.toml` manifest (nearest wins, walking up like git; server-stored per-workspace PROFILES + the local personal manifest complete the layers; `[exclude]` is the one negative state) and a SESSION (user × workspace × installation, `topos login`'s RFC-8628-shaped browser approval → ONE workspace-scoped bearer credential in `identity/sessions.json`, `0600`) is the standing acceptance — delivery is silent from login on, npm-style; `logout` is the self-end (`DELETE /v1/session`), an owner-ended session prints ONE typed line and freezes. `add` is source-polymorphic (workspace refs `@ws/name` / `@ws/channels/x` / bare catalog names edit the nearest manifest — or the `-g` profile route — and deliver in the same invocation; a path adopts in place; `owner/repo` imports at a pin; `add topos` restores the built-in) and `remove` is its inverse (exclude line when a broader layer still provides; two-phase only where loss is real). The bare `update` is the MANIFEST RECONCILE: one delivery call per live session, layers resolved per scope, per-scope placement over per-scope stores (person = shared-dir-first home engine over `~/.topos/`; project = self-ignoring in-checkout dirs over the project's own `.topos/state/<user>/` store), undemanded items cleaned snapshot-first, the applied snapshot reported per session, and a delivery cache (`state/sync_status.json`) keeping `status`/`list` honest offline. The write verbs ride op-WAL idempotent retry through per-session transports; a landed `publish` of a path-ref item TRANSFERS GOVERNANCE by default (catalog entry per protection; the manifest line rewritten to the canonical workspace ref, disclosed); `review`/`protect`/`invite`/`revert`/`log` resolve over the session universe; `auth status` probes each session's health; the offline `status` verb is the trust rail (sessions · this folder's manifests + the profile layers itemized, applied-as-of stamps, excludes as rows · triggers). The BUILT-IN `topos` meta-skill (source `skills/topos/`, embedded, force-synced, `remove topos --yes` opt-out / `add topos` restore) and the `self-update` + passive version check maintenance surface complete it; the whole surface is documented by the generated `docs/cli.md`. | `bins/topos/CLAUDE.md` |
| End-to-end proof | COMPOSED-STACK suites green (the real web app spawned in front of an in-process vault with no public face; the CLI dials the app's `/api` base; every ceremony driven over HTTP — the claim page, the `/verify` login approval, the settings/sessions actions): the SESSION + MANIFEST hero (login → the governance-transferring genesis publish → a project-manifest ref delivering byte-exact into a checkout, self-ignoring → silent fast-forward → protect/downgrade/review-approve → the `-g` profile lane → the owner-side session end printing once and freezing), the deny + logout arms, the NON-ORACLE suite (eight byte-identical uniform-404 misses across foreign/never-existed workspaces, wrong paths, garbage credentials; the `session_approval` knob's born-pending grant with exactly two typed answers until the owner approves), and the HARDENING suite (a pinned `@ws/name@<digest>` reference delivering exactly its version and holding across a sweep; `publish --to` refusing a nonexistent channel — and the workspace-slug near-miss — without minting one). The manifest-layer resolution, reference grammar, and reconcile arms are unit-proven in `bins/topos` over fakes. | `tests/CLAUDE.md` |
