...
needed); the **verb-reshape increment's consumers of this increment's shapes** (the notices ACK route
— the read-state column + fetch-without-ack ride delivery already; the `channel`/`protect`/`follow`/
`unfollow`/`remove` VERBS over the built Authority ops; the describe/narration layer); the **lifecycle
web ceremonies** (archive/unarchive/delete/purge authority ops + guarded functions are BUILT and
session-exposed; the step-up pages are the web increment's); the built-in ACME TLS path's
