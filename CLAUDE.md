...
| Web app | Built (real but early): the product app on React Router 8 (bun) is THE one public surface AND the authority for identity + the whole directory (schema `web`, keyed by `user.id`; email never authorizes). Sign-in (email+password, zero delivery dependency), the gh-style device-approval flow (`/verify` + step-up → the device's ONE bearer credential), workspace create/index/dashboard, the skill pages, the full rendered review UI (approve/reject/withdraw with four-eyes, comments, one-click revert), and the ADMIN surfaces (roster · lifecycle ceremonies · channel admin + history · policy incl. the `registration` knob · fleet with named blind spots · your-devices · the first-run claim) — every admin ceremony STEP-UP-gated (stateless password re-entry; type-the-name on the destructive ones; an `admin_event` row per attempt). **THE DEVICE LANE `/api/v1` is served here and TERMINATES here** — the row ops are Drizzle queries on the app's own `web` schema behind `requireDeviceActor` (the Bearer resolved credential→device→person→seat, hash computed IN Postgres); only the byte/pointer ops of a publish forward to the internal-only vault through the ONE custody transport (`vaultFetch` → `/internal/v1`, a route allowlist); a miss answers the uniform wire 404, a belt wears the 429. First boot mints the workspace + prints the claim link; registration is NEVER open (claim code · invited-on-armed-SMTP · the off-by-default knob). ONE mail transport (invite/verification/reset/magic-link; the passcode is gone). Four composition seams for a downstream superset; single-tenant OSS. Gated by biome/tsc + the boundary/email/token/contract checks + the built-bundle scan, the unit suite, and the Playwright e2e. | `web/CLAUDE.md` |
