...
| Client CLI (15 verbs + 3 maintenance commands) | Built: the accountless local core + crash-safe sidecar, the pull engine (anti-rollback, atomic dir-swap materialization into EVERY placement the shared-dir-first placement engine plans — one `~/.agents/skills` copy for covered detected agents plus native copies for the rest, `--agent`-scopable per skill on this device, draft-anywhere with a typed divergent-copies freeze — diff3 draft resolution, a fast-degrade circuit breaker), no device keypair — the enrolled bearer is the sole credential; `follow` enrolls by workspace ADDRESS (the constant protocol card re-roots onto the bootstrap-declared API base) and `invite` is a roster write; the **device-link** model: a device is REGISTERED once per server (the gh-style browser-approval device flow — ONE Bearer credential per DEVICE into `identity/credentials.json`, a `0600` secret, authenticating EVERY plane request — reads AND writes AND governance) and LINKED per workspace (a first-class severable row; joining a further same-plane workspace is the browser-free two-phase link lane — never a second device flow; a link born `pending` under the workspace's device-approval knob delivers nothing until an owner approves, and the sweep skips it quietly); `auth logout` is ONE global self-revoke (`DELETE /v1/device`); `follows.json` is pure subscription state (a first-run migration scrubs any legacy `read_token`); the write verbs with op-WAL idempotent retry, an un-enrolled `publish` that REFUSES typed (run `follow <workspace-address>` first — workspaces are born in the web app or by the boot claim, not by the CLI), `INVALID_ARGUMENT`/io-kind-honest error codes, `TOPOS_DEBUG=1` + `~/.topos/log.jsonl` diagnostics, `list` **untracked discovery** across the baked 73-harness registry (re-synced from upstream; an opt-in advisory xtask drift check fetches upstream and compares — network, never in ci) (`--tracked` suppresses it) the credentialed **`--remote` catalog** read merged with local follow-state, and **the delivery-driven reconcile**: the bare enrolled sweep runs ONE delivery call per workspace and converges (install new arrivals under their catalog names through the kernel's I-TOFU offer; update against the pre-resolved target; the undelivered remainder splits by WHO ACTED — the person's detached set freezes in place, an upstream withdrawal snapshots any draft and cleans the agent dirs keeping every sidecar byte; a whole-workspace 404 freezes everything, never a clean), then reports the applied snapshot for the fleet page; `publish --to` places channel references; the ancestor backfill shallow-stops at purged history so fresh installs of live descendants survive a purge. **The adopted verb surface** runs ONE resolution grammar (addresses → qualified paths → bare-when-unique) + a consent gate over the built directory ops — two-phase describe/`--yes` where an act has REACH, LOSS, or FIRST-TRUST; immediate with an undo-led receipt where self-scoped and reversible (`--yes` an accepted no-op there — the `--agent` placement scoping, a skill `unfollow`, a followed CLEAN skill's `remove`, and the re-follow of a previously-followed skill incl. the server-side stance clear; a draft holds `remove`'s loss-guard describe, fail-closed on an unscannable copy): `channel add\|remove`, `protect`, `remove` (a per-device exclusion), `unfollow` (the person-scoped detach; the channel arm keeps the computed-breadth describe), the `review` inbox + target describe (the outbox split by the server-computed `yours`; author-withdraw — never `--approve` — offered on your own proposal; a required reject reason), `invite` (a bare `/me` read, else the roster write), `publish`'s enrolled describe (gate outcome · reach · share line · the teammate join line, also on the landed receipt · undo), `update --reset`, `revert`'s two-phase describe + byte-level no-op (tree-compared, so a repeat revert mints nothing), `log`'s purge tombstones (versions deduped, display attribution), `list`'s source/status/cause columns, the `auth login\|logout\|status` group, the offline read-only `status` orientation verb (enrollment · sign-in · follow/offer counts · probed per-agent trigger state · binary version; a bare TTY `topos` renders the same snapshot, or a short welcome on a fresh machine — piped bare invocations keep the usage error), and the two-phase `uninstall` teardown (hook scrub + sidecar removal, dispatched ahead of state recovery); notices ACK by id after an interactive `update` (the quiet hook fetches without acking); the sidecar records per-workspace `sync_status.json` freshness against the served `staleness_window_ms`; `keep-as-yours` re-forks a retained withdrawn copy; the whole surface is documented by the generated `docs/cli.md`. **The BUILT-IN `topos` meta-skill**'s SOURCE is the repo-top-level `skills/topos/` dir (an authored, self-contained `SKILL.md` + `INSTALL.md` + the committed generated `reference.md` = the same bytes as `docs/cli.md` — one renderer, `topos::cli_ref_md()`, drift-gated on BOTH copies), downloadable AS a skill straight from the public repo; the binary embeds those same files and places the same three through the SAME placement engine at the trigger-arming moments, re-syncing force-synced on every bare sweep (its changes ride `reloadSkills`), so a fresh agent learns what topos is, the verb surface, the contribute-back loop, and the team-genesis runbook (create → enroll by address → invite → the teammate join line) from inside its own context; a pre-existing `topos` dir is never written by the sweep; one carrying the public copy's frontmatter provenance marker is adopted snapshot-first by the consented `follow topos --yes` (a marker-less dir stays a frozen Foreign reservation); `remove topos` is a durable device opt-out (`follow topos` re-places), and the name `topos` is reserved where collisions are real (client naming discipline + the app's catalog/channel mints; the workspace slug is deliberately free — the built-in is never a workspace resource). | `bins/topos/CLAUDE.md` |
| End-to-end proof | COMPOSED-STACK suites green (the real web app spawned in front of an in-process vault with no public face; the CLI dials the app's `/api` base; every ceremony driven over HTTP — the claim page, the `/verify` device approval (a plain signed-in accept), the members/settings actions): the full-loop HERO (claim → device-flow enroll → genesis publish → a second device delivers → fast-forward → fleet report → forward revert; table-driven across the Claude Code, OpenClaw, and Hermes adapters), the real `follow` enrollment (card faces, deny arm, uniform misses), the contribute loop (protect → member downgrade → review approve → verdict notices → reject reason → revert), the channels suite (curation propagation, default-channel opt-out/rejoin, device exclusions), the claim suite (printed link, single-use door, the registration knob via the real settings ceremony), the revocation suite (self-revoke immediate + trigger-final + re-enroll, every link severed with cause-tagged audit; seat removal cascades the link away in-request — re-seating alone does not resume delivery, relinking does), the device-links suite (the browser-free second-workspace link with the one-device-row regression witness, self unlink + owner remove freezes, the device-approval knob's pending → approve/reject arms, owner links born active), and a cross-workspace uniform-refusal probe extended to the link gate (a seat without a link answers the same uniform 404). | `tests/CLAUDE.md` |
| Web app | Built (real but early): the product app on React Router 8 (bun) is THE one public surface AND the authority for identity + the whole directory (schema `web`, keyed by `user.id`; email never authorizes). Sign-in (email+password, zero delivery dependency; the magic-link rung when SMTP is armed), the gh-style device-approval flow (`/verify` — a plain signed-in accept), the workspace dashboard, the skill pages, the full rendered review UI (approve/reject/withdraw with four-eyes, comments, one-click revert), and the ADMIN surfaces (roster · lifecycle ceremonies · the tabbed channel pages (skills curation · members · history · settings) · policy incl. the `registration` and device-approval knobs · the Linked-devices tab (link-driven: active links with freshness + per-skill state, the pending approve/reject queue, owner remove — status chips carry tooltip explainers) · your-devices with per-device linked-workspace lists + self unlink · the first-run claim) — every admin ceremony confirms in proportion to its reach (type-the-name on the destructive ones, a lightweight in-place confirm on cross-person acts, plain saves on policy knobs; an `admin_event` row per attempt). **THE DEVICE LANE `/api/v1` is served here and TERMINATES here** — the row ops are Drizzle queries on the app's own `web` schema behind `requireDeviceActor` (the Bearer resolved credential→device→person→seat→live device-link, hash computed IN Postgres); only the byte/pointer ops of a publish forward to the internal-only vault through the ONE custody transport (`vaultFetch` → `/internal/v1`, a route allowlist); a miss answers the uniform wire 404, a belt wears the 429. First boot mints the workspace + prints the claim link (single-tenant only); registration is COMPOSITION-owned — the OSS default stays gated (claim code · invited-on-armed-SMTP · the single-tenant off-by-default knob), a downstream composition may return open. ONE mail transport (invite/verification/reset/magic-link; the passcode is gone) that also writes a metadata-only `mail_event` send log on every send attempt — kind/recipient/outcome only, never a subject or body. Four composition seams for a downstream superset; the URL grammar follows a `tenancy` mode — the OSS default is single-tenant and origin-rooted (the install IS its one workspace), a superset passes `multi` to mount the same modules under the `/:ws` name slug, gaining `/new` self-serve workspace creation (onboarding + the panel dropdown; reserved slugs — the multi-mode statics + the future-reserve list + the composition's own — refuse as taken), multi-workspace device enrollment (the flow row carries the requested slug; approval requires the approver's seat in the resolved workspace), and the vault's per-workspace storage stat over the custody transport. Gated by biome/tsc + the boundary/email/token/contract checks + the built-bundle scan, the unit suite, and the Playwright e2e. | `web/CLAUDE.md` |
