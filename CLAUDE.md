...
**key-rotation-verify** (`KEY_REPIN_REQUIRED` beyond the first pin); the **client half of workspace
standup** (the SERVER half is built — the one-time claim mint/redeem, the standup device flow +
`approve_standup`, `create_workspace`, the `/i/` claim bootstrap; what remains is the client's un-enrolled
`publish` standup resume, `follow <claim-link>`, and the hosted web's verify/create pages that call the
lib-only wrappers); the built-in ACME TLS path's
