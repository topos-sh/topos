...
| Web app | Built (real but early): the product app on React Router 8 (bun) — sign-in (email+password, zero delivery dependency), the device-approval verification page, workspace create/index/dashboard, the members panel (invitation via the guarded `topos_invite`; remove; the review-required toggle), the skill pages (catalog-name-keyed: current files, first-parent history, proposals), the full rendered review UI (diff plan + budgets + sanitized `@pierre/diffs` SSR, approve/reject/withdraw with four-eyes, comments, one-click revert), and the `/i/` claim passthrough + `/install`. Four composition seams for a downstream superset build (`routes()` · nav registry · entitlements · auth config). Row reads = Drizzle SELECTs on schema `plane`; row writes = the guarded `topos_*` functions; byte/pointer ops = the plane's internal session lane. Gated by biome/tsc + the boundary/token/contract checks, 171 unit tests, and a 42-test Playwright e2e (fixture vault + real Postgres). NOT yet: the door cutover (the app does not front `/api/v1`; the plane's public listener stands), compose packaging, roster page depth, lifecycle ceremonies, fleet page. | `web/CLAUDE.md` |
...
needed); the **lifecycle
web ceremonies** (archive/unarchive/delete/purge authority ops + guarded functions are BUILT and
session-exposed; the step-up pages in the web app are still to build); the built-in ACME TLS path's
