...
| Harness adapters | The `HarnessAdapter` port + its three impls: the **Claude Code reference** (discover, adopt-in-place, idempotent `settings.json` SessionStart hook — matcher-free + `async`, so it fires on every source without blocking; its command carries the `--hook claude-code` marker, which is what opts THIS harness into the quiet sweep's `reloadSkills` answer so pulled skills go live same-session; clean uninstall) plus **Hermes** (session-boundary `on_session_start`/`on_session_reset` entries, probed against a real local build; consent-evidence-honest) and **OpenClaw** (native SKILL.md delivery into its default-watched skills root + a silent 1-minute cron trigger registered through its own CLI, declaration-key-idempotent, container-probed live against 2026.7.1; Active only on a successful gateway round-trip; the retired bootstrap-inject artifacts are scrubbed). One sweep everywhere: `topos update --quiet`, self-throttled client-side — registered UNMARKED by every trigger except Claude Code's, because unmarked selects the schema-conservative hook-output document (`hookEventName` + `additionalContext` only, and nothing at all with nothing to say) that a strict validator accepts; a harness proven to understand an extension declares itself with `--hook <harness>`. The crate also carries the baked ~73-harness registry (detection + skills-root resolution), the shared-dir **coverage** module (`~/.agents/skills` support per harness, with live-probed vs vendor-docs provenance; no evidence = not covered), and the one placement-naming discipline every target dir follows. | `crates/topos-harness/CLAUDE.md` |
...
| Client CLI | Built on the MANIFEST architecture: scopes are UNBLENDED — the nearest project `topos.toml` governs its checkout WHOLE (no ancestor merging), and the person scope is this machine's own `~/.topos/topos.toml` when it exists, else the implicit recipe of one FEED row per connected workspace (the feed = the app's assignments minus declines, served whole). A manifest speaks one `[bundles]` namespace whose keys ARE references (the TOML key path joined with `/`, classified by shape: local paths · `github.com/<owner>/<repo>[/<skill>]` · `<host>/<ws>` the feed · `<host>/<ws>/<bundle>` · `<host>/<ws>/channels/<name>`), with `"*"`/a 64-hex version/a 7–40-hex commit/`"off"`/an inline table as values; feed rows and `"off"` are GLOBAL-file-only, and `topos fmt` writes the normal form. A SESSION (user × workspace × installation, `topos login`'s RFC-8628-shaped browser approval → ONE workspace-scoped bearer credential in `identity/sessions.json`, `0600`) is the standing acceptance — delivery is silent from login on, npm-style; `logout` is the self-end (`DELETE /v1/session`), an owner-ended session prints ONE typed line and freezes. **THE CLIENT WRITES NO SERVER STATE** — declines and assignments are web acts; the CLI's one machine-local negative is the `"off"` row. `add` is source-polymorphic (workspace refs `@ws/name` / `@ws/channels/x` / bare catalog names record ONE row in the nearest manifest — or, `-g`, the global file — and deliver in the same invocation; `add -g @ws` adopts a whole feed; a path adopts in place; `owner/repo` imports, gated ONCE on first trust with a member-listing describe — the `-s`/`-a` selectors ride the SAME gate and the same per-scope store, since a selector narrows what lands, never whose bytes it is; `add topos` restores the built-in) and `remove` is its EXACT FILE INVERSE (property-tested: drop the row, write `"off"` when the feed still provides, or offer the set-minus-one rewrite for a channel/repo member — several targets in one set are ONE split; a bare name two rows answer to refuses ambiguous rather than picking, listing the exact `--via <set-ref>` invocations that resolve a set-versus-set tie; two-phase only for LOSS, the set split, or first trust). The bare `update` is the MANIFEST RECONCILE: one delivery call per live session, layers resolved per scope, per-scope placement over per-scope stores (person = shared-dir-first home engine over `~/.topos/`; project = self-ignoring in-checkout dirs over the project's own `.topos/state/<user>/` store — EVERY project-scope path, root and store alike, must provably resolve inside the checkout or it is refused, never redirected), git rows advancing only on an explicit `update` (the quiet sweep never dials a forge) and pins never moving (a pinned repo set is settled only once the member set recorded at landing is fully tracked), undemanded items cleaned snapshot-first per scope — no byte differing from its recorded baseline is destroyed unless a snapshot taken after the last revalidation holds it — the applied snapshot reported per session (one deterministic row per bundle: the person store first, then project stores by path; a cross-store version split is disclosed, not swallowed), and a delivery cache (`state/sync_status.json`) keeping `status`/`list` honest offline. The write verbs ride op-WAL idempotent retry through per-session transports; a landed `publish` of a path-ref item TRANSFERS GOVERNANCE by default (catalog entry per protection; the manifest line rewritten to the canonical workspace ref, disclosed); `review`/`protect`/`invite`/`revert`/`log` resolve over the session universe; `auth status` probes each session's health; the offline `status` verb is the trust rail (sessions · a SECTION per scope with each row's winning reference, source, delivery attribution and applied-as-of stamp · the disclosure notes for inert rows, set collisions, declined-but-delivered bundles · `status <bundle>` for the deep answer · triggers). The BUILT-IN `topos` meta-skill (source `skills/topos/`, embedded, force-synced, `remove topos --yes` opt-out / `add topos` restore) and the `self-update` + passive version check maintenance surface complete it; the whole surface is documented by the generated `docs/cli.md`. | `bins/topos/CLAUDE.md` |
| End-to-end proof | COMPOSED-STACK suites green (the real web app spawned in front of an in-process vault with no public face; the CLI dials the app's `/api` base; every ceremony driven over HTTP — the claim page, the `/verify` login approval, the settings/sessions actions): the SESSION + MANIFEST hero (login → the governance-transferring genesis publish → a project-manifest ref delivering byte-exact into a checkout, self-ignoring → silent fast-forward → protect/downgrade/review-approve → the person-scope FEED lane and its four `-g` arms (redundant-add no-op · the file-birth-then-`off` remove · the sweep's clean · the off-lift) → the owner-side session end printing once and freezing), the deny + logout arms, the NON-ORACLE suite (eight byte-identical uniform-404 misses across foreign/never-existed workspaces, wrong paths, garbage credentials; the `session_approval` knob's born-pending grant with exactly two typed answers until the owner approves), and the HARDENING suite (a pinned `@ws/name@<digest>` reference delivering exactly its version and holding across a sweep; `publish --to` refusing a nonexistent channel — and the workspace-slug near-miss — without minting one). The manifest grammar, the editor's inverse property, per-scope resolution, and the reconcile arms are unit-proven in `bins/topos` over fakes. | `tests/CLAUDE.md` |
| Web app | Built (real but early): the product app on React Router 8 (bun) is THE one public surface AND the authority for identity + the whole directory (schema `web`, keyed by `user.id`; email never authorizes). Sign-in (email+password; the magic-link rung when SMTP is armed), the gh-style login-approval flow (`/verify` — a plain signed-in accept minting the CLI SESSION: user × workspace × installation, workspace-scoped bearer credential), the workspace dashboard, the skill pages (+ the `skills/import` add-from-GitHub flow and the skill-settings Upstream panel), the person's assignments page (`/profile` — Mine, grouped by what puts each skill there: the baseline, each carried channel, what a curator aimed at you, your own picks, every row carrying its attribution and its off switch; and Library, the catalog whose one act adds a skill to your own set) beside `/visibility` (what a workspace can never read from a machine, then what it does, then the reader's own rows as proof), the full rendered review UI (approve/reject/withdraw with four-eyes, comments, one-click revert), and the ADMIN surfaces (roster · lifecycle ceremonies · the tabbed channel pages · policy incl. the `registration` and `session_approval` knobs · the Sessions tab (live sessions + applied state, the pending approve/reject queue, owner remove) · the account Your-sessions list with self end · the first-run claim) — every ceremony confirms in proportion to its reach (an `admin_event` row per attempt). **THE SESSION LANE `/api/v1` is served here and TERMINATES here** — the reads and row ops are Drizzle queries on the app's own `web` schema behind the session guard (Bearer → live `cli_session` → person → seat, hash computed IN Postgres); the login flow at `/api/v1/login/*`, the self-end at `DELETE /api/v1/session` (the retired `/api/v1/workspaces/{ws}/profile*` row ops are GONE — the splat answers the uniform wire 404); only the byte/pointer ops of a publish forward to the internal-only vault through the ONE custody transport (`vaultFetch` → `/internal/v1`, a route allowlist); a miss answers the uniform wire 404 (a PENDING session gets exactly two typed answers: me + delivery), a belt wears the 429. First boot mints the workspace + prints the claim link (single-tenant only); registration is COMPOSITION-owned — the OSS default stays gated. ONE mail transport with the metadata-only `mail_event` send log. Four composition seams + the `tenancy` URL grammar for a downstream superset. Gated by biome/tsc + the boundary/email/token/contract checks + the built-bundle scan, the unit suite, and the Playwright e2e. | `web/CLAUDE.md` |
| Gates + packaging | `cargo xtask ci` = the non-DB CI gates in order; `check-arch` enforces the crate layering, the leaf-crate leanness, the vault's identity-vocabulary + schema-boundary gates (no identity word, no app-schema table in the vault), and the Dockerfile/toolchain pin pair; the compose stack ships the WHOLE product (the web app as the one public surface on :3000; the vault internal-network-only with NO published port; Postgres with the two per-app roles + schemas provisioned at initdb by `scripts/compose-init-db.sh`) and `scripts/compose-smoke.sh` PROVES the shape (no plane host port; the constant card carries the `/api` base; the FIRST document request renders 200 with no retry — the web schema migrates eagerly at boot; the first-boot claim seats an owner whose dashboard reads custody across the schema boundary); `scripts/check-db-grants.sh` proves the cross-lane grants by LOGGING IN as each role; the checksummed echo-then-match installer and the tag-triggered release pipeline (`xtask dist`) ship the rest — with ARMED minisign release signing (the compiled-in `RELEASE_PUBKEY` makes self-update signatures mandatory; CI signs via the `MINISIGN_SECRET_KEY` environment secret; the key ceremony was `scripts/mint-release-key.sh`, the scheme `RELEASING.md`). CI runs the Rust gates + suites, the grants-shape check, the compose smoke, and the web job (checks, vitest, Playwright). | `xtask/CLAUDE.md`, `README.md` |
