...
/// The lock's HEADER is the first thing a reader meets in a committed file, and it must not name
/// a command that did not write it: `topos install` fills entries too, and so does the silent
/// sweep behind an agent's hook. It said "Generated by `topos update`" on every one of them.
#[test]
fn the_lock_header_names_no_verb_that_did_not_write_it() {
    let rig = Rig::new("lock-header");
    rig.seed_session();
    let proj = project(
        "lock-header-proj",
        &format!("workspace = \"{HOST}/{WS_NAME}\"\n\n[skills]\ndeploy = \"latest\"\n"),
    ...
    // The INSTALL path writes this file — nothing here ran `topos update`.
    install(&ctx, &plane, &dir, ops::LockMode::Install).expect("install fills the lock");
    let text = lock_text(&proj.0);
    let header = text.lines().next().unwrap_or_default();
    assert_eq!(
        header, "# Written by topos — the exact versions this project runs; commit it",
        "the file install wrote must not credit another command:\n{text}"
    );
    assert!(
        !text.contains("Generated by"),
        "no verb in the header at all:\n{text}"
    );

    // And an update, which rewrites the whole file, lands the very same line.
    sweep(&ctx, &plane, &dir);
    assert_eq!(
        lock_text(&proj.0).lines().next().unwrap_or_default(),
        header,
        "one header, whichever run wrote it"
    );
}
