...
/// Drive the reshaped `unfollow --yes` over panic connectors — the LOCAL graceful path (no
/// instance.json) never builds a transport, so the panics prove it stays offline.
fn unfollow_local_yes(ctx: &Ctx<'_>, name: &str) -> crate::ops::UnfollowApplied {
    let dir_connect = |_b: &str| -> Box<dyn crate::plane::DirectorySource> {
        panic!("the local unfollow path never builds a directory transport")
    };
    let del_connect = |_b: &str| -> Box<dyn crate::plane::ReconcileTransport> {
        panic!("the local unfollow path never builds a delivery transport")
    };
    let connectors = ops::UnfollowConnectors {
        directory: &dir_connect,
        delivery: &del_connect,
    };
    match ops::unfollow(ctx, &connectors, &[name.to_owned()], &[], &[], true).unwrap() {
        ops::UnfollowOutcome::Applied(a) => a,
        ops::UnfollowOutcome::Described { .. } => panic!("--yes applies"),
    }
}
...
fn unfollow_flips_follow_state_keeps_bytes_and_is_idempotent() {
    ...
    assert_eq!(u.items[0].kind, "skill");
    assert_eq!(u.items[0].name, "pr-describe");
    assert!(u.bytes_kept);
    // (The golden `--json` fixture for the reshaped two-phase unfollow is regenerated by the tests
    // leg alongside the other new-verb fixtures.)
    ...
    let u2 = unfollow_local_yes(&ctx, "pr-describe");
...
fn unfollow_of_a_tracked_but_never_followed_skill_is_a_clean_success() {
    ...
    // An unknown name is the ONE uniform not-found, never a silent success.
    let dir_connect = |_b: &str| -> Box<dyn crate::plane::DirectorySource> {
        panic!("nothing enrolled — no directory transport")
    };
    let del_connect = |_b: &str| -> Box<dyn crate::plane::ReconcileTransport> {
        panic!("nothing enrolled — no delivery transport")
    };
    let connectors = ops::UnfollowConnectors {
        directory: &dir_connect,
        delivery: &del_connect,
    ...
    assert!(matches!(
        ...
        Err(crate::error::ClientError::TargetNotFound { .. })
    ));
}
...
fn list_discloses_enrollment_follow_state_and_hook() {
    ...
    let out = ops::list(&h.ctx_with(&hooked), None, false, None, None).unwrap();
...
fn follow_approve_resumes_an_unfollowed_skill() {
    ...
    let del_connect2 = |_b: &str| -> Box<dyn crate::plane::ReconcileTransport> {
        unreachable!("the skill path builds no delivery transport")
    };
    ...
    assert_eq!(data.skills[0].name, "pr-describe");
    assert_eq!(resumed, vec!["pr-describe".to_owned()]);
    let text = render::follow_tty(&data, &resumed);
    assert!(
