...
/// Read `map.json` — the ONE persisted document with its own schema ceiling
/// ([`PLACEMENT_MAP_SCHEMA_VERSION`], the per-placement `placement_state` shape). Every other
/// document keeps [`read_doc`]'s [`PERSISTED_SCHEMA_VERSION`] dispatch; the same fail-closed rule
/// applies here (a document above the ceiling is an error, never silently parsed or deleted).
///
/// # Errors
/// As [`read_doc`]; additionally [`ClientError::Corrupt`] when a document's `placement_state` is
