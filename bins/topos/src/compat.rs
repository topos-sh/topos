...
/// wire in a way a client CANNOT DEGRADE THROUGH — this one: an MCP server is no longer a bundle
/// of bytes but a row the delivery answer carries INLINE, in a list of its own, and an older
/// server has no such list. This build would read every connected server as withdrawn and take
/// its config entries out of every agent on the machine. Refusing the old server here names the
/// fix instead. Removals a client rides out on its own do not move this floor; a break with no
/// graceful degradation moves it in the same change.
pub(crate) const MIN_SERVER_VERSION: &str = "0.1.42";
