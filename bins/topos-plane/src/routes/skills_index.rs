...
//! workspace MEMBER, authorized by a DEVICE-signed read (catalog visibility == workspace membership, on BOTH
//! cloud and self-host). The reading device names its key in the `Topos-Device-Key-Id` header and rides its
//! 64-byte catalog-read signature (over `(ws, device_key_id)`) in `Topos-Device-Signature` (base64url,
//! decoded exactly as the write routes decode it). A missing/malformed header, an unknown/revoked device, a
//! bad signature, or a non-member is the single indistinguishable **404** (never a 400/401/403). The catalog
//! is MUTABLE (a publish moves a pointer; a proposal opens/closes) and membership-varying, so it carries only
//! a short, `private` must-revalidate window.
...
#[utoipa::path(
    ...
    params(
        ...
        ("Topos-Device-Signature" = String, Header, description = "base64url(64-byte Ed25519 catalog-read signature over (ws, device_key_id)), 86 chars."),
    ),
    responses(
        ...
        (status = 404, description = "Missing/malformed header, unknown/revoked device, bad signature, or non-member (indistinguishable).", body = JsonEnvelope),
...
pub(crate) async fn list_skills(
    ...
    let signature = wire::read_signature(&headers)?;
    let index = state
        .list_skills_device(&ws, &device_key_id, &signature)
        .await?;
