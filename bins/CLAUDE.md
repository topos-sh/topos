...
Each is a **lib + a thin bin**: the lib holds the logic (unit-testable without a binary), the bin is
a thin composition root.

- **`topos/`** — the CLIENT. The CLI an agent drives; the full verb reference is generated into
  `docs/cli.md`. Depends on the kernel + gitstore + the harness port. **Takes no dependency on
  `plane-store` / `sqlx`** — it is a thin sync tool, never an authority.
